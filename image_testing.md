# Image Integration Testing Rules — AeroSentinel
...
## Endpoint under test
- `POST /api/complaints` — accepts JSON `{ image_base64, ward_id, location_text }`. Returns `{ id, analysis: { detected, confidence, severity, description, recommended_action }, ... }`. Backed by Gemini 3 Flash Vision via emergentintegrations.
