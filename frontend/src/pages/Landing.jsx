...
import { ResponseTimeHero } from "@/components/ResponseTimeHero";
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-white">
      ...
      <main className="max-w-[1600px] mx-auto px-6 lg:px-8 py-8 space-y-10" data-testid="landing-main">
        ...
        <section>
          <SectionLabel kicker="Eval Criterion · Business Impact (25%)" title="Signal-to-intervention latency." />
          <ResponseTimeHero />
        </section>
        ...
        {topHotspot && (
          <section className="bg-[#141414] border border-[#EF4444]/40 rounded-sm p-6 lg:p-8">
            ...
            <div className="mt-5 flex items-center gap-3 flex-wrap">
              <Link to="/command" data-testid="landing-to-command" className="flex items-center gap-2 font-mono-data text-xs uppercase tracking-wider px-4 py-2.5 bg-white text-black hover:bg-[#22C55E] rounded-sm transition-colors">
                Open Command Center <ArrowRight className="w-3.5 h-3.5" />
              </Link>
              <Link to="/citizen" data-testid="landing-to-citizen" className="flex items-center gap-2 font-mono-data text-xs uppercase tracking-wider px-4 py-2.5 border border-white/20 hover:border-white/50 text-white/80 hover:text-white rounded-sm transition-colors">
        ...
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <PortalCard to="/command" testid="portal-card-command" icon={ShieldAlert} kicker="Operations" title="Command Center" desc="Geospatial source attribution map, FIRMS thermal anomaly overlay, intervention simulator with Execute Plan. The pollution war room." />
            <PortalCard to="/citizen" testid="portal-card-citizen" icon={User} kicker="Citizens · 3M+ served" title="Citizen Portal" desc="Bilingual ward-level advisory, evidence-based complaint submission with AI-analysed photos, real-time risk." />
            <PortalCard to="/inspector" testid="portal-card-inspector" icon={Hammer} kicker="Field" title="Inspector Queue" desc="Sorted dispatch list with evidence trace, auto-drafted notices, and live Auto-Plan queue from the simulator." />
          </div>
        </section>

        <section>
          <SectionLabel kicker="Architecture" title="Multi-agent AI under the hood." />
          <AgentBadges />
        </section>
        ...
        <footer className="pt-10 pb-6 border-t border-white/5 text-xs font-mono-data text-white/40 uppercase tracking-wider flex items-center justify-between flex-wrap gap-3">
          ...
          <span>Forecast → Attribution → Enforcement → Advisory · Vision · Copilot</span>
        </footer>
      </main>
...
const SectionLabel = ({ kicker, title }) => (
  <div className="mb-5">
    <div className="font-mono-data text-[10px] uppercase tracking-[0.22em] text-white/50 mb-2">{kicker}</div>
...
const CitywideStatusCard = ({ kpis }) => {
  ...
  return (
    <div className="bg-[#141414] border border-white/10 rounded-sm p-6">
      <div className="font-mono-data text-[10px] uppercase tracking-[0.18em] text-white/50 mb-3">Citywide · right now</div>
      <div className="flex items-baseline gap-4 mb-4">
        <div className="font-mono-data text-6xl font-semibold tabular tracking-tight" style={{ color: bandColor(kpis.city_band) }}>{kpis.city_aqi}</div>
        <div>
          <div className="font-mono-data text-[10px] uppercase tracking-wider px-2 py-0.5 inline-block rounded-sm border" style={{ color: bandColor(kpis.city_band), borderColor: `${bandColor(kpis.city_band)}66`, background: `${bandColor(kpis.city_band)}1a` }}>
...
const PortalCard = ({ to, testid, icon: Icon, kicker, title, desc }) => (
  <Link to={to} data-testid={testid} className="group bg-[#141414] border border-white/10 hover:border-white/40 rounded-sm p-6 transition-colors block">
    <div className="flex items-start justify-between mb-4">
      <div className="font-mono-data text-[10px] uppercase tracking-[0.22em] text-white/50">{kicker}</div>
